...
def spin(balance):
    ...
    slots = get_slot_machine_spin(rows=ROWS, cols=COLS, symbols=symbols_count)
    print_slot_machine(columns=slots)
    winnings, winning_lines = check_winnings(
        columns=slots, lines=lines, bet=bet, values=symbols_value
    )
    print(f"\nYou won ${winnings}.")
    if winnings > 0:
...
def main():
    ...
    while True:
        ...
        if balance == 0:
            print("Your Balance is out of cash, go to top up again")
            break
